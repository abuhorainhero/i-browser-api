...
const withdrawalRequestUpdate = async (req, res, next) => {
  try {
    const info = req.body;
    const { id } = req.params;
    const withdrawalRequestUp = await withdrawalRequest_update(id, info);
    if (!withdrawalRequestUp) {
      return res.status(400).json({
        error: true,
        withdrawalRequest: null,
        message: "Withdrawal Request not updated!"
      })
    }
    const withdrawalRequest = await withdrawalRequest_get_one(id);
    const withdrawalRequestObj = JSON.parse(JSON.stringify(withdrawalRequest));
...
const withdrawalRequestDelete = async (req, res, next) => {
  try {
    const { id } = req.params;
    //console.log(id);
    const withdrawalRequestDel = await withdrawalRequest_delete(id);
    if (!withdrawalRequestDel) {
      return res.status(400).json({
        error: true,
        withdrawalRequest: null,
        message: "Withdrawal Request not deleted!"
      })
    }
    const withdrawalRequest = await withdrawalRequest_get_one(id);
    const withdrawalRequestObj = JSON.parse(JSON.stringify(withdrawalRequest));
