...
const withdrawalMethodUpdate = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    //console.log(id, info);
    const withdrawalMethodUp = await withdrawalMethod_update(id, info);
    if (!withdrawalMethodUp) {
...
const withdrawalMethodDelete = async (req, res, next) => {
  try {
    const { id } = req.params;
    const withdrawalMethodDel = await withdrawalMethod_delete(id);
    if (!withdrawalMethodDel) {
      return res.status(400).json({
        error: true,
        withdrawalMethod: null,
